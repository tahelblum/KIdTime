...
// קומפוננטה להוספת ילד
export const AddChildForm = ({ authToken }: { authToken: string }) => {
  ...
  const handleAddChild = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://[ה-URL-שלך-מ-XANO]/api:wZUcfmuE/add_child', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${authToken}` // שולח את הטוקן של ההורה המחובר
        },
  ...
  return (
    <div className="p-4 border rounded-lg bg-gray-50 shadow-sm max-w-sm">
      <h3 className="text-lg font-semibold mb-2 text-right">הוספת ילד חדש</h3>
